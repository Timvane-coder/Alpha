...
class serialize {
	...
	async send(content, options = {}, type = 'text') {
		...
		if (type == 'text') {
			...
			return poll;
		} else if (type == 'button') { // Add handling for sending button message
            // Create interactive message data
            const createInteractiveMessage = require("../utils")
            const data = {
                jid: this.jid,
                button: content.buttons, // Assuming the buttons are provided in the content
                header: content.header, // Assuming the header is provided in the content
                footer: content.footer, // Assuming the footer is provided in the content
                body: content.body // Assuming the body is provided in the content
            };
            // Create interactive message
            const interactiveMessage = await createInteractiveMessage(data, options);
            // Send the message
            return await this.client.sendMessage(this.jid, interactiveMessage, options);
        }
	} 
}
module.exports = {
