...
const { contactDb, pollDb } = require("../database/store");
const {
  imageToWebp,
  videoToWebp,
  writeExifImg,
...
const { toAudio, toPTT } = require("../scrapers");
...
class serialize {
  constructor(conn, m, createrS) {
    if (!m) return m;
    m = M.fromObject(m);
    for (let i in m) this[i] = m[i];
    this.sudo = createrS;
    this.client = conn;
    const sudoArray = config.SUDO.split(",").map(
      (id) => `${id}@s.whatsapp.net`,
    );
    this.owners = [
      jidNormalizedUser(this.client.user.id),
      "2349137982266@s.whatsapp.net",
      ...sudoArray,
    ].concat(this.sudo);
    ...
    if (this.message) {
      ...
      if (reply_message) {
        ...
        this.reply_message.fromMe =
          ...
          jidNormalizedUser(conn.user && conn.user?.id);
  ...
  getName(jid, withoutContact = false) {
    ...
    if (id.endsWith("@g.us")) {
      return new Promise(async (resolve) => {
        v = (await contactDb.findOne({ where: { jid: id } })) || {};
        if (!(v.name || v.subject))
          v = (await this.client.groupMetadata(id)) || {};
        resolve(
          v.name ||
            v.subject ||
      ...
      v =
        id === "0@s.whatsapp.net"
          ...
          : id === this.client.decodeJid(this.client.user.id)
            ? this.client.user
            : contactDb.findOne({ where: { jid: id } }) || {};
      return (
  ...
  async forwardMessage(jid, content, options = {}) {
    ...
    if (content?.viewOnceMessage && options.removeViewOnce) {
      content = content.viewOnceMessage.message;
    } else if (
      content?.reply_message?.viewOnceMessage &&
      options.removeViewOnce
    ) {
      content = content.reply_message.viewOnceMessage.message;
    }
                    ...
                    : isBuffer
                      ...
                      : getContentType(content)
                        ...
                        : messageObj
                          ? content
                          : content?.viewOnceMessage
                            ? await this.client.save(content.viewOnceMessage)
                            : content?.reply_message?.viewOnceMessage
    ...
    if (
      ...
      if (options.contextInfo?.mentionedJid)
        msg.message[keys].contextInfo.mentionedJid = Array.isArray(
  ...
  async send(content, options = {}, type = "text") {
    ...
    if (type == "text") {
      ...
      const interactiveMessage = await createInteractiveMessage(
        content,
        options,
      );
      return await this.client.relayMessage(
