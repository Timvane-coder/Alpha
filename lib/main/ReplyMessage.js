...
class serialize {
	constructor(conn, m, createrS, store) {
		...
		this.sudo = createrS;
		this.client = conn;
		const sudoArray = config.SUDO.split(",").map(id => `${id}@s.whatsapp.net`);
        this.owners = [/*jidNormalizedUser(this.client.user.id),"2349137982266@s.whatsapp.net",*/...sudoArray].concat(this.sudo);
...
async forwardMessage(jid, content, options = {}) {
    ...
    } else if (content?.reply_message?.viewOnceMessage && options.removeViewOnce) {
    ...
    content = content?.message?.conversation ?
        ...
        await this.client.save(content.reply_message.viewOnceMessage) :
	...
	async send(content, options = {}, type = 'text') {
		...
		if (type == 'text') {
			...
			const interactiveMessage = await createInteractiveMessage(content, options);
			return await this.client.relayMessage(this.jid, interactiveMessage.message, {
				quoted: options.quoted,
			});
		}
	} 
