...
const pollDb = config.DATABASE.define("poll", {
  pollKey: {
    type: DataTypes.STRING,
    allowNull: false,
    primaryKey: true,
...
const loadMessage = async (jid, id) => {
  ...
  if (message) return message.dataValues.message;
  return false;
};
...
const savePoll = async (pollKey, data) => {
  try {
    let exists = await pollDb.findOne({ where: { pollKey } });
    if (exists) {
      return await pollDb.update({ data }, { where: { pollKey } });
    } else {
      return await pollDb.create({ pollKey, data });
    }
  } catch (e) {
    console.log(e);
  }
...
const loadPoll = async (pollKey) => {
  const poll = await pollDb.findOne({
    where: { pollKey },
  });
  if (poll) return poll.dataValues.data;
  return false;
};
