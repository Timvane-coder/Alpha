...
const { WarnsDB, saveWarn, getWarns, resetWarn } = require("./database/warn");
  ...
  const Alpha = async () => {
    try {
      ...
      conn.ws.on("CB:call", async (json) => {
        if (json.content[0].tag == "offer") {
          ...
          if (config.CALL_BLOCK) {
            ...
            await conn
              .sendMessage(callfrom, { text: config.ANTI_CALL_BMSG })
              .catch((e) => console.log(e));
            await conn
            ...
            await conn
              ...
              .catch((e) => console.log(e));
            await conn
              .sendMessage(callfrom, { text: config.ANTI_CALL_MSG })
              .catch((e) => console.log(e));
          }
        }
      });
      ...
      conn.ev.on("messages.upsert", async (m) => {
        const { pdm, antilink, antiword, } = await groupDB(["pdm","antilink","antiword",],{ jid: m.messages[0].key.remoteJid }, "get");
